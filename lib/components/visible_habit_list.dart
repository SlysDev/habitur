...
class VisibleHabitList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool userHasChosenToShareHabits =
        habitsScope == SharingScope.everyone ||
    ...
    if (userId == AuthService().currentUser!.uid || userHasChosenToShareHabits) {
  ...
  Widget _buildHabitList(List<Habit> habits) {
    final List<Habit> visibleHabits = habits.where((habit) => habit.isVisible ?? false).toList();
                ...
                children: [
                  ...
                  DifficultyRatingStatChip(
                    difficultyRating: habit.stats.length > 0
                      ? double.parse(habit.stats.last.difficultyRating.toStringAsFixed(2))
                      : 0.0
                  ),
                ],
              ),
