...
import 'dart:math';
import '../stat_change_indicator/stat_change_indicator.dart';
...
class LineGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final niceInterval = [1, 2, 5, 10]
        ...
        .firstWhere((interval) => interval >= rawInterval)
        .toDouble();

    // Determine number of decimal places based on interval size
    final decimalPlaces = niceInterval >= 0.1
        ? 1
        : niceInterval >= 0.01
            ? 2
            : 3;
    ...
    final adjustedMaxY =
        ((maxY / niceInterval).ceil() * niceInterval) + niceInterval;
                ...
                child: LineChart(
                  LineChartData(
                    gridData: FlGridData(
                      ...
                      verticalInterval: dateInterval.toDouble(),
                    ...
                    borderData: FlBorderData(
                      show: true,
                      border: Border(
                        bottom: BorderSide(color: kGray.withOpacity(0.2)),
                        left: BorderSide(color: kGray.withOpacity(0.2)),
                      ),
                    ),
                    minX: spots.first.x,
                    maxX: spots.last.x,
                    ...
                    lineTouchData: LineTouchData(
                      touchTooltipData: LineTouchTooltipData(
                        tooltipBgColor: kDarkGray.withOpacity(0.8),
                        getTooltipItems: (List<LineBarSpot> touchedBarSpots) {
                          return touchedBarSpots.map((barSpot) {
                            final date = DateTime.fromMillisecondsSinceEpoch(
                                barSpot.x.toInt());
                            return LineTooltipItem(
                              '${date.month}/${date.day}: ${barSpot.y.toStringAsFixed(decimalPlaces)}',
                              TextStyle(
                                color: Colors.white,
                                fontSize: 12,
                              ),
