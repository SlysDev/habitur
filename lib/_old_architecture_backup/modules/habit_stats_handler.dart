...
class HabitStatsHandler {
  ...
  Future<void> incrementCompletion(context,
    ...
    try {
      ...
      debugPrint(
          'Starting habit completion at: ${stopwatch.elapsedMilliseconds}ms');
      ...
      await Future.wait([
        Future(() async {
          if (userStorage.currentUser.stats.isEmpty) {
            ...
            debugPrint(
                'Stats loading took: ${stopwatch.elapsedMilliseconds - loadStart}ms');
          }
        }),
      ...
      if (habit.currentProgress == habit.targetGoal) {
        ...
        updateOperations.addAll(activities
            .map((activity) => activityProvider.createActivity(activity)));

        debugPrint(
            'Completion preparation took: ${stopwatch.elapsedMilliseconds - completionStart}ms');
      }
      ...
      debugPrint(
          'Local stats update took: ${stopwatch.elapsedMilliseconds - statsStart}ms');
      ...
      debugPrint(
          'Remote updates took: ${stopwatch.elapsedMilliseconds - updateStart}ms');
    } finally {
      debugPrint(
  ...
  void _updateExistingStatPoint(
      int index, double recordedDifficulty, Map<String, double> slopes) {
    final statPoint = habit.stats[index];
    statPoint.completions++;
    statPoint.streak = habit.streak;
    statPoint.consistencyFactor =
  ...
  void _createNewStatPoint(
      double recordedDifficulty, Map<String, double> slopes) {
  ...
  Future<void> decrementCompletion(context) async {
    ...
    if (habit.isCommunityHabit) {
      await db.statsDatabase.updateAllStats(
          AuthService().currentUser!.uid, UserLocalStorage().currentUser.stats);
      return;
    }
    ...
    if (habit.currentProgress == 0) {
      ...
      if (currentDayIndex != -1) {
        ...
        habit.stats[currentDayIndex].consistencyFactor = _calculator
  ...
  Future<void> setDifficulty(double newDifficulty, context) async {
    ...
    if (currentDayIndex != -1) {
      ...
      StatPoint newStatPoint = StatPoint(
        ...
        streak: habit.streak,
        consistencyFactor: _calculator.calculateConsistencyFactor(
      ...
      if (habit.stats.indexWhere((dataPoint) =>
        ...
        StatPoint newStatPoint = StatPoint(
          ...
          consistencyFactor: isOffDay
              ...
              : _calculator.calculateConsistencyFactor(
                  habit.stats, habit.targetGoal),
          ...
          slopeConsistency: isOffDay
              ...
              : _calculator.calculateStatSlope(
