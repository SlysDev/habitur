...
class ActivityProvider with ChangeNotifier {
  ...
  Future<void> _loadInitial() async {
    ...
    try {
      ...
      _activities.clear();
      final snapshot =
          await _activityDb.loadInitialActivities(user.uid, user.friends);
  ...
  Future<void> createActivity(ActivityEvent activity) async {
    debugPrint(
        '-------------------- Running createActivity() with activity: ${activity.toString()}');
    final UserModel user = userStorage.currentUser;
    ...
    switch (activity.type) {
      case ActivityType.habitCompletion:
        if (!user.privacySettings!.shareHabitCompletions) {
          debugPrint(
              'Not creating habit completion activity because habit completions are not shared');
          return;
        }
        break;
      case ActivityType.streakMilestone:
        if (!user.privacySettings!.shareStreakMilestones) {
          debugPrint(
              'Not creating streak milestone activity because streak milestones are not shared');
          return;
        }
        break;
      case ActivityType.newHabit:
        if (!user.privacySettings!.shareNewHabits) {
          debugPrint(
              'Not creating new habit activity because new habits are not shared');
          return;
        }
        break;
    }
