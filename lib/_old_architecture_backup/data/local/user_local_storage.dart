...
class UserLocalStorage extends ChangeNotifier {
  ...
  Future<void> populateDefaultUserData() async {
    ...
    try {
      ...
      debugPrint(
          'Friend Requests: ${currentUser.receivedFriendRequests.length} received, ${currentUser.sentFriendRequests.length} sent');
  ...
  Future<void> updateUserStats(
      Map<String, dynamic> updates, BuildContext context) async {
    ...
    try {
      ...
      updates.forEach((key, value) {
        switch (key) {
          ...
          case 'confidenceLevel':
            currentUser.stats![currentDayIndex].confidenceLevel =
                value as double;
            break;
          case 'streak':
          ...
          case 'consistencyFactor':
            currentUser.stats![currentDayIndex].consistencyFactor =
                value as double;
            break;
          case 'difficultyRating':
            currentUser.stats![currentDayIndex].difficultyRating =
                value as double;
            break;
          case 'slopeCompletions':
            currentUser.stats![currentDayIndex].slopeCompletions =
                value as double;
            break;
          case 'slopeConsistency':
            currentUser.stats![currentDayIndex].slopeConsistency =
                value as double;
            break;
          case 'slopeConfidenceLevel':
            currentUser.stats![currentDayIndex].slopeConfidenceLevel =
                value as double;
            break;
          case 'slopeDifficultyRating':
            currentUser.stats![currentDayIndex].slopeDifficultyRating =
                value as double;
            break;
        }
      });
      ...
      notifyListeners();
      debugPrint(
          'Batch user stats update took: ${stopwatch.elapsedMilliseconds}ms');
    } finally {
      stopwatch.stop();
    }
  }
