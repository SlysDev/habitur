...
class HabitDatabase {
  ...
  Future<void> uploadHabits(context, {String? userID}) async {
    try {
      ...
      var habitsCollectionSnapshot = await _habitsCollection.get();
  ...
  Future<void> updateHabit(Habit habit, context) async {
    ...
    try {
      ...
      final habitQuery = await _habitsCollection
          .where('id', isEqualTo: habit.id)
          .limit(1)
          .get();
      debugPrint(
          'DB Query took: ${stopwatch.elapsedMilliseconds - queryStart}ms');
      ...
      batch.update(habitDoc.reference, {
        ...
        'lastSeen': habit.lastSeen,
        'daysCompleted':
            habit.daysCompleted.map((date) => {'date': date}).toList(),
        'stats': dataConverter.dbStatPointsToMap(habit.stats),
      });
      ...
      debugPrint(
          'Batch preparation took: ${stopwatch.elapsedMilliseconds - batchStart}ms');

      // Track commit time
      final commitStart = stopwatch.elapsedMilliseconds;
      await batch.commit();
      debugPrint(
          'Batch commit took: ${stopwatch.elapsedMilliseconds - commitStart}ms');

      Provider.of<NetworkStateProvider>(context, listen: false).isConnected =
          true;
      debugPrint(
          'Total database operation took: ${stopwatch.elapsedMilliseconds}ms');
    } catch (e, s) {
