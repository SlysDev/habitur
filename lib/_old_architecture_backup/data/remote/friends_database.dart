...
class FriendsDatabase {
      ...
      () async {
        ...
        for (var request in receivedRequests) {
          if (friendRequest.equals(request)) {
            debugPrint(
                '📤 Updating current user doc to remove friend request...');
            await currentUserDoc.update({
  ...
  Future<void> cancelFriendRequest(
    ...
    await showStatusOverlay(
      ...
      () async {
        debugPrint(
            '🔄 Starting cancelFriendRequest for request: ${friendRequest.toFirebaseMap()}');

        // Get recipient's document
        DocumentReference? recipientDoc =
            await getUserDocById(friendRequest.recipientUid);
        if (recipientDoc == null) {
          throw Exception('Recipient not found');
        }
    ...
    await showStatusOverlay(
      ...
      () async {
        ...
        List<FriendRequest> receivedFriendRequests = (userSnapshot
                ...
                ?.map(
                    (req) => FriendRequest.fromMap(req as Map<String, dynamic>))
                .toList() ??
            [];
        debugPrint(
            '📤 Loaded received friend requests: $receivedFriendRequests');
  ...
  Future<List<Habit>> getFriendVisibleHabits(
      String friendUid, BuildContext context) async {
    try {
      debugPrint(
          '📊 Starting getFriendVisibleHabits for friendUid: $friendUid');
      return await Database()
          .habitDatabase
          .loadHabits(context, userID: friendUid);
    } catch (e, s) {
